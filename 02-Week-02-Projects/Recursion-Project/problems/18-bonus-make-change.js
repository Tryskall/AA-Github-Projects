...
function makeBetterChange(target, coins = [25, 10, 5, 1], index = 0) {
  if (target === 0) {
      return [];
  }
  ...
  coins.sort((a, b) => a - b);
  const withCoin = makeBetterChange(target - coins[index], coins, index);
  const withoutCoin = makeBetterChange(target, coins, index + 1);

  if (withCoin !== null && (withoutCoin === null || withCoin.length < withoutCoin.length)) {
      return [coins[index], ...withCoin];
