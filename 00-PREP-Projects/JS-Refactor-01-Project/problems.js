...
function countConsonants(word) {
    ...
    let consonants = word.length - count;
    return consonants;
}
...
//     const seperate = strLow.split(' ');
//     const backTogether = [''];
//     for (let i = 0; i < str.length(); i++) {
//         if (seperate[i] % 2 == 0) {
//             seperate[i].toUpperCase();
//             backTogether.push(seperate[i]);
//         }
//         else {
//             backTogether.push(seperate[i]);
//         }
...
function alternatingLetters(str) {
  uPlOw = str.split('').map( (letter, i) =>
  (i % 2) == 0 ? letter.toLowerCase() : letter.toUpperCase() ).join('');
  return uPlOw;
}
alternatingLetters("alphabeticallyupanddown")
